...
| ID | Sev | Issue | Status |
|----|-----|-------|--------|
| B1 | **P0** | Dual-database split: `users` table created in SQLite, queried in Postgres | Worked around (`_init_users.py`) |
| B2 | P1 | `postgresql.JSONB` hardcoded → app cannot run on SQLite despite docs | Forced Postgres |
| B3 | **P0** | Audit rows written without their parent rows → FK violation / orphaned audit trail | Local hack (dropped FKs) — **must revert** |
| B4 | P1 | ChromaDB guideline collections not seeded → RAG pipeline init fails | **Confirmed** |
| B5 | **P0** | Stdlib loggers called with structured-logging kwargs (`error=`, `db_path=`) → `TypeError` 500s every request; **defeats an intentional graceful fallback** | Open (needs rebuild) |
| B6 | **P0** | `decision_id` is **generated by the LLM** but stored in a `unique=True` column → repeat submissions of the same case collide → `IntegrityError` → 500. Session not rolled back, so the failure is masked by a `PendingRollbackError` | **Confirmed** (reproduced 2026-07-22) |
...
*Workaround applied:* created the `users` table in Postgres via `src/pacca/_init_users.py`.
...
- Revert the local FK drop (B3) and implement the atomic request→decision→audit transaction.
- Consolidate auth onto the single async Postgres engine (B1).
- Fix the logging-API mismatch that 500s every submission (B5) and seed ChromaDB (B4) — together these are why no decision can currently complete.
- Move `decision_id` generation server-side and out of the LLM response schema (B6) — a resubmitted case currently 500s, and a reused id cross-links audit trails.
- Real-database integration + API contract tests (test Phases 1–2).
- Secrets out of source (A5/C6); audit-log immutability (§3.3).

**P1:**
- Commit to Postgres-only; fix/remove SQLite claims (B2).
...
- `src/pacca/_init_users.py`: one-off creation of the `users` table in Postgres (delete once B1 is fixed properly).
