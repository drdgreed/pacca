...
| **v2.5** | **2026-07-20 (this PRD)** | **Records the dataset crossing the 100-case production-pilot threshold (105 as-built, GC-001–GC-105). Refreshed §16.2 (evaluation surface + verified outcome distribution), §16.3 (sufficiency — 10pp detection now met, coverage floor met), §16.4 (power table met-at-105 column), §16.6 (coverage at 105), §16.8 (production-pilot marked reached; roadmap to 300/500+), and §16.9 (honest-claims matrix — 10pp and DENY-sufficiency now Yes). Reworked §16.7 into a two-stage CRB model: formation at 100 (in progress, via the Foundation Partner — a Chicago-based charitable foundation named under NDA — with pediatric-specialized case expansion) and operational scored κ-sweeps at 200. Corrected iteration status (iter-6 closed/tagged, iter-7 seeded on the off-label↔experimental finding), prompt-registry versions (decision_support v2.6, medical_director v2.2), and test count (590). Reconciles the v2.5 Google-Doc draft, which had projected a 100-case/board-operational state that did not match the repo.** |
