...
## Part VI — Triage pipeline: the GC-020 regression & decouple

### ADR-020 · Decouple triage from the Tier-1 decision (supersedes the enrichment half of ADR-016)
**Context.** ADR-016 fed Evidence + Classification into the DecisionAgent's prompt as "advisory"; ADR-019 made the golden-20 gate a re-baseline to catch movement. The re-baseline earned its keep: run through the *actual enriched path* (not the bare DecisionAgent the CI gate exercises), **GC-020** — an oncological emergency (ALL + leukostasis; expected AUTO_APPROVED) — regressed from score **5 → 2**. Variant isolation proved **classification alone** causes it: the DecisionAgent reads "complexity 5 / URGENT / requires specialist review" as an escalation directive and flips AUTO_APPROVED → IN_REVIEW (confidence unchanged at 0.98). Evidence-only stays 5.
**Decision.** **Decouple triage from the Tier-1 decision.** Drop `DecisionContext.evidence`/`classification`, the orchestrator's `model_copy` enrichment, and the DecisionAgent's triage prepend. Keep both triage agents running in `_run_triage` for the **audit trail + routing/queue metadata** — decision-support for humans, never the approve/escalate call.
**Why.** *Severity is not uncertainty.* The only clinically valid trigger for human escalation is decision-uncertainty/risk (owned by the deterministic detector + confidence routing), not patient acuity. Feeding routing-severity language into the decision made the system *more* hesitant for the most acute patients — for GC-020, a review delay inserted into a life-threatening emergency. From the patient's view that delay is the dominant harm. The decoupled decision returns to the proven iter-6 baseline (mean 4.9) with zero residual risk.
**Alternatives.** (1) *Evidence-only* enrichment — keeps a decision input with no measured golden-20 upside and a plausible "evidence-gaps → over-escalate/deny" downside; needs a fresh re-baseline to trust. (2) *Prompt-guardrail* the DecisionAgent to ignore severity — fragile; rests patient safety on the model obeying "ignore the signal I'm feeding you." Both rejected for the simpler, proven, more auditable path.
**Consequences.** Triage is decision-support, not a decision input — ADR-016's *audit-trail* rationale stands; its *prompt-enrichment* rationale is superseded. A regression-guard unit test asserts the DecisionAgent receives the original un-enriched context. Two follow-ups: (a) the CI clinical gate exercises the bare DecisionAgent — which post-decouple matches production again — but a gate over `orchestrator.process_decision` end-to-end would have caught this sooner; (b) triage runs on the decision's critical path for audit-only value — consider moving it off-path later.
